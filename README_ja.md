...
- アンインストール機能
- **変更されたスキルを更新** - ルート行の標準ボタン、またはコマンドパレットの「全管理ルートの変更されたスキルを更新」で配布元を比較します。変更なしならローカル編集・比較情報・出力ファイルを書き直しません。変更分は件数と上書き注意を確認後、一時領域で検証して置き換えます。
- **配布元での名前変更** - 元のパスが消えた場合、記録済み commit と現在の GitHub revision を比較して `SKILL.md` の名前変更を一意に確認できたときだけ、別名でのインストールを提案します。旧コピーは残し、確認なしには導入しません。曖昧・未検証の変更は保留します。導入が成功して表示・出力更新だけ失敗した場合はスキル出力を再生成します。不完全な導入は別途通知します。
- **索引未登録の新しいスキル** - 新名でインストールしたスキルは、ソース索引への反映前でも記録済みの配布元 revision と照合します。内容が変わらなければ変更なし、上流が変わっていれば索引の更新まで保留します。
- **初回同期と修復** - 比較情報のない既存スキルは別途同意した場合だけ初回同期します。不完全・出所不明・確認失敗は保留します。従来の全件・ルート再インストールは修復用として右クリックメニューと既存コマンドに残ります。
...
- **ルート単位 inline アクション** - 書き込み可能な各 skill root 行の右端に **スキル出力を再生成** を表示します。これは `AGENTS.md` / `copilot-instructions.md` / `CLAUDE.md` または `ref` モードのリンク先 catalog を再生成する操作です。さらに、remote-backed skill を 1 件以上含む root にだけ **このルートの変更されたスキルを更新** を表示します
...
- 書き込み可能な各 root 行の右端にも **スキル出力を再生成** が表示され、remote-backed skill を 1 件以上含む root にだけ **このルートの変更されたスキルを更新** が表示されます
...
- ツールバー: スキル出力 / スキル出力を再生成 / 新規作成 / ビューを更新 / 設定
- 書き込み可能な各 root 行の右端にも **スキル出力を再生成** が表示され、remote-backed skill を 1 件以上含む root にだけ **このルートの変更されたスキルを更新** が表示されます。これで GitHub Copilot Home / Claude Home / Global Agent Home などをコマンドパレットなしで更新できます
...
| `Agent Skills Ninja: カテゴリで検索`                                | カテゴリを選んでスキルを絞り込む                                                                                              |
...
| `Agent Skills Ninja: お気に入りを表示`                              | お気に入りに入れたスキルだけを表示                                                                                            |
| `Agent Skills Ninja: 最近インストールしたスキル`                    | 直近にインストールしたスキルを表示                                                                                            |
...
| `Agent Skills Ninja: 孤児マーカーブロックを掃除`                    | 出力先に残った skill-ninja / resource-ninja / agent-ninja のマーカーブロックを削除する                                        |
| `Agent Skills Ninja: スキルフォルダを開く`                          | インストール済みスキルのフォルダを開く                                                                                        |
| `Agent Skills Ninja: 中断したソース更新を再開`                      | GitHub のレート制限や 1 回あたりの更新上限で持ち越した source 更新を再開する                                                  |
...
| `Agent Skills Ninja: 共存ステータスを表示`                          | Agent Resources Ninja との共存モードと、現在どちらが出力先を書いているかを表示する                                            |
| `Agent Skills Ninja: 共存オーナーを再評価`                          | 共存オーナーの判定をやり直す（sibling を入れた / 外した直後に使う）                                                           |
...
コマンドパレットに出るコマンドのうち、次はツリーの行を右クリックしても実行できる。

| 操作                   | 対象                       |
| ---------------------- | -------------------------- |
| スキルをインストール   | リモートスキルの行         |
| バンドルをインストール | バンドルの行               |
| スキルを再インストール | 導入済みスキルの行         |
| スキルをプレビュー     | リモートスキルの行         |
| SKILL.md を開く        | 導入済みスキルの行         |
| 説明を編集             | 導入済みスキルの行         |
| お気に入りを切り替え   | リモートスキルの行         |
| GitHub で開く          | スキル / ソースの行        |
| URLをコピー            | スキル / ソースの行        |
| パスをコピー           | 導入済みスキル / root の行 |
| ターミナルで開く       | 導入済みスキル / root の行 |
...
|  11  | `skillNinja.outputTargets`                | _(未設定)_                 | 出力先ごとの ON/OFF と形式の上書き。未設定なら検出した全出力先へ書き、明示的な `[]` はどこにも書かない     |
...
- **リトライには上限があります。** 一括実行の後、一時的なサーバー・通信エラーだけを自動で 1 回、さらに手動で最大 1 回再試行します。どちらも既存フォルダーを削除しません。認証失敗、レート制限、`404`、ポリシー上限、分類不能のエラーは同じ再試行を案内し続けません。失敗通知の **バグ報告** では機密値を除いた診断情報をプレビューし、確認後に GitHub の Issue 下書きを開きます。自動投稿は行いません。
- **一部の再試行成功で、ほかの失敗を隠しません。** 再試行対象外の失敗も最終警告と Issue 診断に残します。認証、SSO、PAT ポリシー、部分取得の分類は保持し、エラー原文や個人パスは含めません。
- **読取失敗をファイル不在と扱いません。** 読取と存在確認の両方が失敗した場合、instruction / catalog の出力やローカル登録で既存データを置き換えません。登録・解除では壊れたメタデータも保持します。JSON を修復するかバックアップから戻して再実行してください。インストール途中で既存メタデータを再読込できない場合も、そのメタデータを上書きせず失敗とします。
- **同じスキルフォルダーの変更は一つずつ実行してください。** 差分更新は同じ Extension Host 内の重複トランザクションを拒否しますが、ウィンドウ間のロックではありません。別ウィンドウや別ツールから、同じ実体のフォルダーを同時に更新・再インストール・削除しないでください。
...
- **配布元が同梱した `.skill-meta.json` は使いません。** このファイルは拡張機能が所有します。リポジトリに含まれていても安全でないファイル名として数えず静かに無視し、記録するインストール位置は必ず走査が実際に見つけた場所から再計算します。
...
差分更新では、読込・ルート走査・更新有無の確認・適用の段階を進捗表示します。ローカルスキルだけの場合など、リモート更新対象がなければ GitHub の認証情報を確認せず終了します。

ビルド後、インストール済みの VS Code を使って英日スモークテストを再実行できます。
...
実行器はホーム・AppData・設定・拡張保存先・ワークスペースを一時領域へ分離し、認証情報を引き継ぎません。自分で起動したホストの終了後に一時データを削除します。実際の拡張有効化、保守コマンド登録、古いルート情報・引数なしの処理、ローカルのみの更新を繰り返しても変更しないことを検証します。通常プロファイルへのインストール、実 GitHub ダウンロード、マウスクリック、画面レイアウトの検証は行いません。`npm test` は VS Code を開かず隔離と終了処理の契約を検証します。起動方法は [公式の拡張テスト実行API](https://code.visualstudio.com/api/working-with-extensions/testing-extension#the-test-runner-script) に従います。

個人のスキルルートではなく、使い捨てワークスペースとホームプロファイルを持つ分離した Extension Development Host で確認してください。自動テストに加えて、以下の UI 確認が必要です。

1. 起動直後・ツリー更新直後に、使い捨てルートの更新ボタンを押します。選んだルートが保持され、別ルートへ切り替わらないことを確認します。
2. 配布元の同梱資料だけを変更し、一度更新した後、変更を加えず再度更新します。初回だけ対象スキルが置き換わり、ほかのスキルと二回目の実行では変更がないことを確認します。
3. 継続する失敗とキャンセルを試します。再試行上限、対象外の失敗の保持、**バグ報告** の診断プレビューを確認します。確認をキャンセルした場合、外部の Issue 画面が開かないことも確認します。
4. 使い捨てコピーで出力の読取不能とメタデータの破損を試します。内容が保持されることを確認し、権限や JSON を修復して再実行します。出力できない場合は **詳細を表示** 付きの警告が出て、`Agent Skills Ninja: Skill State` を開けます。ローカル診断に対象の出力グループと `unreadable`、`locked`、`failed` の状態が記録されるため、権限やロックを解消して再生成してください。

出力の再生成では、更新・変更なし・無効・他拡張への委譲・失敗を区別します。出力に失敗した手動再生成で成功表示は出しません。スキル自体のインストール成功とは別に、出力生成の警告が出る場合があります。同じ出力先・状態での自動警告は、復旧または明示的な再生成まで重複抑止します。診断のパスはローカルに留め、Issue へ自動送信しません。隔離スモークテストでも、読めない出力先の保護と正常な再生成への復旧を検証します。
